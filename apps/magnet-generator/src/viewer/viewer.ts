// three.js viewer for the magnet generator: Z-up CAD frame, ACES tone mapping,
// room-environment PBR, a pickable part layer. Adapted from the clicker generator's
// viewer (no section/explode/switch modes — the magnet is a single-flip part).
...
/** front = the image face (camera above), back = the magnet face (camera below),
 *  iso = the default three-quarter view. */
export type ViewPreset = 'front' | 'back' | 'iso';

export interface MagnetHandleOptions {
  /** Pocket centres in body coordinates, mm. */
  positions: [number, number][];
  /** The magnet's own radius, mm — drawn as the inner circle. */
  magnetRadius: number;
  ...
  size?: [number, number];
  /** Per-magnet rotation in degrees (blocks). */
  rotations?: number[];
  /** Which one is selected, or -1. */
  active: number;
  /** Interactive (magnet mode) draws them bright; otherwise they're a faint guide. */
  interactive: boolean;
}
...
export interface Viewer {
  ...
  setParts(parts: MagnetPart[], refit?: boolean): void;
  /** Frame the model from a named angle. */
  setView(preset: ViewPreset): void;
  /** Screen point -> body coordinates (mm from the model centre), or null if the
   *  ray misses the model. Used to place magnets by clicking on the part. */
  ...
  /** Turn off left-drag orbit so magnet mode can place instead of spin. */
...
export function createViewer(container: HTMLElement): Viewer {
  ...
  function setParts(parts: MagnetPart[], refit = false) {
    clearPlaceholder();
    clearGroup(root);
    materials.length = 0;
    partMeshes.length = 0;
  ...
  // ---- Magnet markers: the pocket outline plus the magnet itself, drawn over the
  ...
  function outlinePoints(o: MagnetHandleOptions, rot: number): THREE.Vector2[] {
    const pts: THREE.Vector2[] = [];
    if (o.outline === 'rect') {
      const [w, h] = o.size ?? [o.magnetRadius * 2, o.magnetRadius * 2];
      ...
      // Same construction as the pocket: inradius = the magnet's radius.
      const r = o.magnetRadius / Math.cos(Math.PI / 6);
      for (let i = 0; i < 6; i++) {
        const a = (Math.PI / 3) * i;
      ...
      for (let i = 0; i < 48; i++) {
        ...
        pts.push(new THREE.Vector2(Math.cos(a) * o.magnetRadius, Math.sin(a) * o.magnetRadius));
      }
    }
    return pts;
  }
  ...
  function setMagnetHandles(o: MagnetHandleOptions) {
    handlePositions = o.positions;
    handleRadius = Math.max(o.magnetRadius, 2);
    clearHandles();
    ...
    o.positions.forEach((p, i) => {
      ...
      // The magnet itself — always a circle for a disc, and always INSIDE the
      // hex outline, which is the point of the hex profile.
      if (o.outline === 'hex') {
        const circle: THREE.Vector3[] = [];
        for (let s = 0; s <= 48; s++) {
          const a = (Math.PI * 2 * s) / 48;
          circle.push(new THREE.Vector3(Math.cos(a) * o.magnetRadius, Math.sin(a) * o.magnetRadius, 0));
        }
      ...
      if (o.interactive) {
        const fill = new THREE.Mesh(
          new THREE.CircleGeometry(o.magnetRadius, 32),
  ...
  function handleAt(clientX: number, clientY: number): number | null {
    ...
    // A little slop so small magnets are still easy to grab.
