...
import { button, numberField, segmentedControl, sliderRow } from '@vostok/ui-kit';
...
export function runWizard(opts: WizardOpts) {
  ...
  function stepPreprocess() {
      ...
      <div class="wz-modal lg">
        ...
        <div class="wz-body">
          ...
          <div class="wz-controls">
            ...
            <div id="wzThickMount"></div>

            <div class="wz-label">Image Adjustment</div>
            ${SLIDERS.map(([k]) => `<div id="wzAdj-${k}"></div>`).join('')}
          </div>
        </div>
        <div class="wz-foot">
    ...
    const ratioRow = segmentedControl<CropRatio>({
      options: RATIOS.map(([k, l]) => ({ value: k, label: l })),
    ...
    overlay.querySelector('#wzRatioMount')!.replaceWith(ratioRow);

    const thickRow = numberField({
      label: 'Image Thickness',
      value: params.thicknessMm,
      min: 0.2,
      max: 10,
      step: 0.2,
      unit: 'mm',
      onInput: (v) => (params.thicknessMm = v),
    });
    overlay.querySelector('#wzThickMount')!.replaceWith(thickRow);
    ...
    for (const [k, l] of SLIDERS) {
      ...
      overlay.querySelector(`#wzAdj-${k}`)!.replaceWith(row);
    }
  }
