...
- **Export goes through the host** — instead of a browser download, the two-colour 3MF is sent
...
`src/makerlab/glue.js`, `makerlab/config.json`, and `makerlab/pack.mjs` are our own code and
are committed.

## Commands

```bash
# Produce the submission zip (dist/ + config.json) → makerlab/keycap-legend-generator-mw.zip
npm run pack:mw
