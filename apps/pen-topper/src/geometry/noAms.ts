...
export function noAmsPauses(
  params: Pick<TopperSettings, 'colorScheme' | 'style' | 'plateThickness' | 'haloThickness' | 'layerHeight'>,
  ...
  const { base, halo } = snapLayers(params.plateThickness, params.haloThickness, params.layerHeight);
  if (hasHalo) {
    return [
