...
export function placeNet(net: Net, margin: number): Net {
  ...
  return {
    ...net,
    panels: net.panels.map((p) => ({
      ...p,
      outline: t(p.outline),
      holes: p.holes.map(t),
      // `windowRect` is net coordinates like everything else here, and a field this
      // function forgets is a field that silently means something different before
      // and after placement.
      ...(p.windowRect
        ? {
            windowRect: [p.windowRect[0] + dx, p.windowRect[1] + dy, p.windowRect[2], p.windowRect[3]] as [number, number, number, number],
          }
        : {}),
    })),
