...
// vertical or horizontal layout, deterministic always-fused keyring.
...
// Plate border around the letters (Plate style)
...
// Same plate height for every name — lines up on a display rack
...
// Plate expands the lug by plate_margin (pre-shrink to compensate); 3D uses closing which
// doesn't expand, so the lug is already final size there.
...
dy        = (text_size + line2_sz) * 0.62;  // vertical offset between the two lines

// ---- 2D text profile (deterministic origin: no textmetrics in 2021.01) -------
module row(txt, sz, y)
    translate([gap, y])
        text(txt, size = sz, font = the_font, halign = "left", valign = "center", spacing = sp,
...
// the top-left of the name. A hull bridge to a point INSIDE the name guarantees the ring
// always fuses to the plate, whatever the leading glyph or plate_margin.
corner = ring_style == "Corner hole";
ring_cx = layout == "Vertical" ? (corner ? text_size*0.28 : 0)
        : (corner ? gap + lug_outer*0.6 : lug_outer);
ring_cy = layout == "Vertical" ? (corner ? -text_size*0.1 : text_size*0.55 + lug_outer*0.5)
...
module tab_2d()
    hull() {
        translate([ring_cx, ring_cy]) circle(r = lug_pre);
        translate([bridge_x, bridge_y]) circle(r = 0.8);
    }

// Uniform height (horizontal): a hidden pad bar forces a constant plate height across names
// with/without ascenders & descenders. Width uses a len() estimate — only HEIGHT need be exact.
est_w = max(nchars, 1) * text_size * 0.62;
module uniform_pad_2d()
    translate([gap + est_w/2, 0]) square([est_w, text_size*1.24], center = true);
...
module connector_2d() {
    if (layout == "Vertical")
        translate([0, -(nchars-1)*vstep/2]) square([text_size*0.35, nchars*vstep], center = true);
    else
        translate([gap + est_w/2, 0]) square([est_w, text_size*0.42], center = true);
}

// Thin bar at the left joining the two lines (and the ring) into one piece while keeping
// the rest of each line's outline contoured.
module line_link_2d()
    translate([gap, 0]) square([3, dy + text_size*0.4], center = true);

module plate_src_2d() {
    glyphs_2d();
    tab_2d();
    if (uniform_height && layout == "Horizontal") uniform_pad_2d();
